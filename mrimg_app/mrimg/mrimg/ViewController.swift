...
class ViewController: NSViewController, NSComboBoxDelegate {
    ...
    override func viewDidLoad() {
        ...
        sys.path.append("/Users/atulphadke/Documents/Intel_Work/Imgsoftware")
        let classPath = Python.import("main")
        sys.path.append("/Users/atulphadke/Documents/Intel_Work/Imgsoftware/nibabel")
        nib = Python.import("nibabel")
    ...
    func convert() {
        ...
        if directory_analysis {
            for (idx, file) in bruker2dseq_files.enumerated() {
                classPath.convert(file1_type.stringValue, file2_type.stringValue, file.path, self.plantDirectory.path+"/"+self.file2 + "\(idx)")
            }
        } else {
        
            classPath.convert(file1_type.stringValue, file2_type.stringValue, self.file1.path, self.plantDirectory.path+"/"+self.file2)
        }
        let alert = NSAlert()
